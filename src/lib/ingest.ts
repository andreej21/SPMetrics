  ...
  for (const ev of incoming) {
    ...
    const inserted = await db
      ...
      .values({
        ...
        spmSource: session.spmSource ?? null,
...
async function resolveVisitor(
  ...
  const [created] = await db
    ...
    .values({
      ...
      firstSpmSource: attr.spmSource ?? null,
      firstSpmVersion: attr.spmVersion ?? null,
      firstSpmExperiment: attr.spmExperiment ?? null,
      firstSpmChannel: attr.spmChannel ?? null,
      firstSeenAt: now,
      lastSeenAt: now,
    })
...
async function resolveSession(
  ...
  const [created] = await db
    ...
    .values({
      ...
      spmSource: attr.spmSource ?? null,
      spmVersion: attr.spmVersion ?? null,
      spmExperiment: attr.spmExperiment ?? null,
      spmChannel: attr.spmChannel ?? null,
