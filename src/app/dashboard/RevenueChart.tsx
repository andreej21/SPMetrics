  ...
  return (
    <div ref={wrapRef} className="chart" onMouseMove={onMove} onMouseLeave={() => setHi(null)}>
      <svg viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none" style={{ width: "100%", height: 220, display: "block" }}>
        ...
        {hiPt && (
          <>
            <line x1={x(hi!)} x2={x(hi!)} y1={padTop} y2={H - padBottom} stroke="var(--border)" strokeWidth="1" vectorEffect="non-scaling-stroke" />
            <circle cx={x(hi!)} cy={y(hiPt.revenue)} r="3.5" fill="var(--accent)" stroke="var(--plane)" strokeWidth="2" vectorEffect="non-scaling-stroke" />
          </>
        )}
      </svg>
